                    ...
                    int initial_id, vector<vector<int>> &motif_neighbor, vector<vector<int>> &matched_subgraphs)
{
    ...
    if (InitCandidate)
    {
        ...
        if (motif_neighbor[initial_id].empty() && !current_node_set.empty())
        {
    ...
    FindMaximalMotifClique_new(label2adj, label2cover, subgraph, motif, graph, current_node_set, candidates, maximal_motif_cliques,
             ...
             vector<vector<int>> &matched_subgraphs)
{
    ...
    for (unsigned int i = 0; i < matched_subgraphs.size(); ++i)
    {
        ...
        GetMMC_VF3_new(motif, graph, matched_subgraphs[i], i, motif_neighbor, matched_subgraphs);
    }
