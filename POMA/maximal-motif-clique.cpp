...
void GetCandidates(const map<int, map<int, set<int>>> &label2adj, const set<int> &current_node_set,
                   const Graph &motif, const Graph &graph, unordered_set<int> &candidates)
{
            ...
            for (auto neighbor : graph.adjList[id])
            {
                ...
                if (!IsVisited[neighbor])
                {
                    ...
                    if (label2adj.find(label) != label2adj.end())
                    {
                        ...
                        flag = false;
                        int count = 0;
                        for (const auto elementAdj : label2adj.at(label))
                        {
                   ...
                   unordered_set<int> &candidates_new)
{
    ...
    for (const auto &nid : candidates)
    {
        ...
        if (!flag)
        {
            for (const auto element : label2cover.at(label))
            {
                if (graph.adjList[nid].find(element) == graph.adjList[nid].end())
                {
                    cover_flag = false;
        ...
        else if (graph.labels[new_node] == label)
        {
            ...
            else if (IsomorphismCheck(current_node_set, new_node, nid, motif, graph))
            {
                candidates_new.insert(nid);
            }
        }
                                ...
                                int initial_id, vector<vector<int>> &motif_neighbor, vector<vector<int>> &matched_subgraphs)
{
    ...
    for (auto from : matched_subgraph)
    {
        ...
        for (auto from : matched_subgraph)
            for (auto to : matched_subgraph)
            {
                ...
                {
                    if (label2adj.find(graph.labels[from]) == label2adj.end())
                    {
                        label2adj[graph.labels[from]].insert(make_pair(from, set<int>()));
                    }
                    label2adj[graph.labels[from]][from].insert(to);

                    if (label2adj.find(graph.labels[to]) == label2adj.end())
                    {
                        label2adj[graph.labels[to]].insert(make_pair(to, set<int>()));
                    }
                    label2adj[graph.labels[to]][to].insert(from);
                    // update label2cover:
                    if (label2cover.find(graph.labels[from]) == label2cover.end())
                    { // label[from] does not exist in label2adj
                        label2cover.insert(make_pair(graph.labels[from], set<int>()));
                    }
                    label2cover[graph.labels[from]].insert(to);

                    if (label2cover.find(graph.labels[to]) == label2cover.end())
                    { // label[to] does not exist in label2adj
                        label2cover.insert(make_pair(graph.labels[to], set<int>()));
                    }
                    label2cover[graph.labels[to]].insert(from);
                }
            }
    }
    ...
    if (!ORDER)
    {
        ...
        {
            candidates.insert(new_subgraph_node);
        }
    }
    else
    {
    ...
    DFSMaximal_new(label2adj, label2cover, current_node_set, motif, graph, candidates,
                   NOT, maximal_motif_cliques, initial_id, motif_neighbor, matched_subgraphs);
    return;
}
...
inline int GetPivot(const unordered_set<int> &candidates, set<int> NOT)
{
...
int Pick(const set<int> &current_node_set, const Graph &motif, const Graph &graph, const unordered_set<int> &candidates, const unordered_set<int> &early_checks,
         const set<int> &NOT)
{
    ...
    if (EarlyStopCheck(current_node_set, motif, graph, early_checks, NOT))
    {
        return -1;
    }
    return GetRandomElement(candidates);
...
inline void GetPruningSets(int pivot, const unordered_set<int> &candidates, unordered_set<int> &erase_sets)
{
    bool flag;
    bool cover_flag;
    for (const auto &nid : candidates)
    {
        ...
        if (!UseAP)
        {
            if (IsomorphismCheck(current_node_set, pivot, nid, motif, graph))
...
void UsePivotPruning(const set<int> &NOT, const unordered_set<int> &candidates, unordered_set<int> &candidates_copy, int choice)
{
    ...
    for (const auto &e : current_node_set)
    {
        if (graph.adjList[pivot].find(e) != graph.adjList[pivot].end() && motif_labelAdj[label].find(graph.labels[e]) != motif_labelAdj[label].end())
        {
            ...
            label2adj[graph.labels[e]][e].insert(pivot);
            new_set.insert(e);
            label2cover[graph.labels[e]].insert(pivot);
            if (label2cover[label].find(e) == label2cover[label].end())
            {
                cover_set.insert(e);
                label2cover[label].insert(e);
            }
            if (label2adj[label].find(pivot) == label2adj[label].end())
            {

                label2adj[label][pivot].insert(e);
            }
            else if (label2adj[label][pivot].find(e) == label2adj[label][pivot].end())
            {
                label2adj[label][pivot].insert(e);
            }
        }
    ...
    GetPruningSets(pivot, candidates, erase_sets);
    current_node_set.erase(pivot);
    erase_sets.erase(pivot);
    // Recover
    if (label2cover[label].find(pivot) != label2cover[label].end())
        label2cover[label].erase(pivot);
    for (auto &e : label2cover[label])
    {
        label2cover[graph.labels[e]].erase(pivot);
    }
    for (auto &e : cover_set)
    {
        label2cover[label].erase(e);
    }
    label2adj[graph.labels[pivot]].erase(pivot);
    for (auto &e : new_set)
    {
        label2adj[graph.labels[e]][e].erase(pivot);
    }

    for (const auto &e : erase_sets)
        candidates_copy.erase(e);
    Pruning_vertices_sp += (candidates.size() - candidates_copy.size());
    All_vertices_sp += 1;
}
...
{
    if (MMC_LIMIT != -1 && maximal_motif_cliques.size() > MMC_LIMIT)
        return;
    ...
    if (candidates.empty() && NOT.empty())
    {
        ...
        mclique_size += current_node_set.size();
    ...
    set<int> NOT_new;
    unordered_set<int> candidates_new;
    unordered_set<int> candidates_copy(candidates);
    ...
    if (PIVOT != 0)
    {
        UsePivotPruning(NOT, candidates, candidates_copy, PIVOT);
    }
    ...
    while (!candidates_copy.empty())
    {
        ...
        double start = clock();
        new_node = Pick(current_node_set, motif, graph, candidates_copy, candidates, NOT);
        ...
        ticks = clock();
        if (ORDER || !FindEndPointInSetTrieNode(current_node_set))
        {
            set<int> new_set;
            set<int> cover_set;
            ...
            for (const auto &e : current_node_set)
            {
                if (graph.adjList[new_node].find(e) != graph.adjList[new_node].end() && motif_labelAdj[label].find(graph.labels[e]) != motif_labelAdj[label].end())
                {
                    ...
                    label2adj[graph.labels[e]][e].insert(new_node);
                    label2cover[graph.labels[e]].insert(new_node);
                    if (label2cover[label].find(e) == label2cover[label].end())
                    {
                        cover_set.insert(e);
                        label2cover[label].insert(e);
                    ...
                    if (label2adj[label].find(new_node) == label2adj[label].end())
                    {
                        ...
                        label2adj[label][new_node].insert(e);
                    }
                    else if (label2adj[label][new_node].find(e) == label2adj[label][new_node].end())
                    {
                        // new_set.insert(e);
                        label2adj[label][new_node].insert(e);
                    }
            ...
            if (!candidates_new.empty() || NOT_new.empty())
                DFSMaximal_new(label2adj, label2cover, current_node_set, motif, graph, candidates_new, NOT_new,
                               maximal_motif_cliques, initial_id, motif_neighbor, matched_subgraphs);

            // Recover label2cover.
            if (label2cover[label].find(new_node) != label2cover[label].end())
                label2cover[label].erase(new_node);
            for (auto e : label2cover[label])
            {
                label2cover[graph.labels[e]].erase(new_node);
            }
            for (auto e : cover_set)
            {
                label2cover[label].erase(e);
            ...
            // Recover label2adj.
            label2adj[graph.labels[new_node]].erase(new_node);
            for (auto e : new_set)
            {
                label2adj[graph.labels[e]][e].erase(new_node);
            }
        }
